...
public class HashDualMap<P, S, V> implements DualMap<P, S, V> {
    ...
    public HashDualMap(final @Range(from = 0, to = Integer.MAX_VALUE) int initialCapacity) {
    ...
    @Override
    public P getPrimaryKey(final @Nullable S secondary) {
        return this.keyMap.get(secondary);
    }

    @Override
    public S getSecondaryKey(final @Nullable P primary) {
        final var entry = this.map.get(primary);
        return entry == null ? null : entry.getKey();
    }

    @Override
    public V getByPrimaryKey(final @Nullable P primary) {
        final var entry = this.map.get(primary);
        return entry == null ? null : entry.getValue();
    }

    @Override
    public V getBySecondaryKey(final @Nullable S secondary) {
