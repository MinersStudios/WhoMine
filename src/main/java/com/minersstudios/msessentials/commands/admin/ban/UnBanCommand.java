...
public class UnBanCommand implements MSCommandExecutor {
    ...
    public @NotNull List<String> onTabComplete(
        ...
        if (args.length == 1) {
            ...
            sender.getServer().getBanList(BanList.Type.PROFILE).getEntries().stream()
            .map(entry -> (PlayerProfile) entry.getBanTarget())
            .filter(playerProfile -> playerProfile.getName() != null && playerProfile.getId() != null)
            .forEach(playerProfile -> {
                UUID uuid = playerProfile.getId();
                String name = playerProfile.getName();
                PlayerInfo playerInfo = PlayerInfo.fromProfile(uuid, name);
